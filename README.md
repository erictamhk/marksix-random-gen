...
本程式無需安裝任何伺服器或依賴庫 (No dependencies)，下載即用。

1.  **下載代碼**：
    Clone 這個 repo 或者直接下載 ZIP 檔案。
    ```bash
    git clone https://github.com/erictamhk/marksix-random-gen.git
    ```
2.  **開啟網頁**：
    在文件夾內找到 `index.html`，雙擊用任何瀏覽器（Chrome, Safari, Edge 等）打開。
3.  **產生號碼**：
